...
// Favorites
const favorites = ref<string[]>(['/usuarios', '/metricas'])

const toggleFavorite = (path: string | null) => {
  if (!path) return
...
<template>
  <aside
    :class="[
      'flex fixed h-screen inset-y-0 left-0 z-30 flex-col transition-all duration-300 ease-in-out',
      'border-r backdrop-blur-sm bg-white/95 dark:bg-gray-900/95 border-gray-100/40 dark:border-gray-800/40',
      'shadow-[0_0_15p_rgba(0,0,0,0.01)] dark:shadow-[0_0_15px_rgba(0,0,0,0.1)]',
      store.isCollapsed ? 'md:w-[84px]' : 'w-72'
    ]"
  >
    <!-- Logo Section -->
    <div
      :class="[
        'flex items-center h-16 px-5 shrink-0 border-b border-gray-100/70 dark:border-gray-800/40',
        store.isCollapsed ? 'justify-center' : 'justify-between'
      ]"
    >
      <div class="flex items-center space-x-3 overflow-hidden">
        ...
        <div
          :class="[
            ...
            store.isCollapsed ? 'opacity-0 w-0' : 'opacity-100'
          ]"
        >
          <span
      ...
      <div v-if="!store.isCollapsed" class="flex items-center space-x-1">
        <button
          @click="toggleSearch"
        ...
        >
          ...
          <span
            class="absolute top-0 right-0 h-2 w-2 rounded-full bg-rose-500 ring-2 ring-white dark:ring-gray-900"
          ></span>
        </button>
      </div>
    </div>
    ...
    <div
      ...
      class="px-5 py-4 border-b border-gray-100/70 dark:border-gray-800/40"
    >
    ...
    <nav class="flex-1 overflow-y-auto py-4 px-3 scrollbar-thin">
      <div class="space-y-7">
        ...
        <div v-if="favorites.length > 0 && !store.isCollapsed" class="relative">
          <div class="mb-3">
            <h3
          ...
          <div class="space-y-1">
            ...
            >
              ...
              <component
                :is="
                  allNavigationItems.find((item) => item.route === path)?.icon
                "
              ...
              <span class="truncate">
                {{
                  allNavigationItems.find((item) => item.route === path)?.name
                }}
              </span>

              <div
                @click.stop="toggleFavorite(path)"
                class="ml-auto text-amber-400 opacity-0 group-hover:opacity-100 transition-opacity"
              >
                <StarIcon class="h-3.5 w-3.5 fill-amber-400" />
              </div>
            </button>
          </div>
        </div>
          ...
          <div class="space-y-1">
            ...
            >
              ...
              >
                ...
                >
                  <component
                    ...
                    :class="[
                      ...
                      store.isCollapsed ? 'h-6 w-' : 'h-5 w-5 ',
                      activeRoute === item.route
                        ? 'dark:text-primary-400 text-primary-600'
                        : 'dark:text-gray-400 dark:group-hover:text-gray-200 text-gray-500 group-hover:text-gray-900'
                    ]"
                ...
                <!-- Favorite Star -->
                <div
                  v-if="!store.isCollapsed && !item.badge"
                  @click.stop="toggleFavorite(item.route)"
                  class="ml-auto opacity-0 group-hover:opacity-100 transition-opacity p-1"
                >
                  <StarIcon
                    class="h-3.5 w-3.5"
                    :class="
                      favorites.includes(item.route || '')
